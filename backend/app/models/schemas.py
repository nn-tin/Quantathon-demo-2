...
class HybridConfig(BaseModel):
    ...
    shots: int = Field(default=256, ge=128, le=10000)
    optimizer_shots: int = Field(default=64, ge=64, le=4096)
    optimizer_evaluations: int = Field(default=6, ge=4, le=100)
    top_k: int = Field(default=10, ge=2, le=100)
    max_quantum_rounds: int = Field(default=2, ge=1, le=2)
    random_seed: int = 7
    quantum_target: str | None = "nvidia"
...
class RunConfig(BaseModel):
    ...
    run_mode: str = "hybrid_demo"
    scenario_input: ScenarioInput | None = None
    # Retained internally so benchmark code can provide block dimensions and a
    # seed. The public demo endpoint overwrites all QAOA hyperparameters with
    # the immutable GPU profile.
