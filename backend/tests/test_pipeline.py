...
def _small_config(scenario_input=None) -> RunConfig:
    return RunConfig(scenario_input=scenario_input)


class _DeterministicQuantumBackend:
    def solve(self, problem, config):
        bits = str(problem.metadata["incumbent_bitstring"])
        candidate = CandidateBitstring(
            rank=1,
            bitstring=bits,
            energy=evaluate_qubo_energy(problem, bits),
            sample_count=config.hybrid_config.shots,
            probability=1.0,
            hamming_distance_from_incumbent=0,
            source="test_gpu_stub",
        )
        return BackendResult(
            backend=BackendKind.HYBRID,
            status="completed",
            mode_label="GPU test stub",
            best_energy=candidate.energy,
            candidates=[candidate],
            raw_payload={
                "target": "nvidia",
                "execution_device": "gpu",
                "execution_backend": "test_stub",
            },
            backend_runtime_ms=1.0,
        )
...
def test_one_request_returns_hybrid_only_on_gpu_profile():
    service = PipelineService()
    service.quantum_backend = _DeterministicQuantumBackend()
    summary = service.execute_run(_small_config(_frontend_scenario()))
    assert summary.status.startswith("completed")
    assert summary.result is not None
    assert set(summary.result) >= {"hybrid", "recommended_plan", "convergence"}
    assert "classical" not in summary.result
    assert "comparison" not in summary.result
    assert summary.result["recommended_plan"]["id"] == "hybrid"
    assert summary.result["hybrid"]["round_count"] >= 1
    assert summary.result["hybrid"]["active_qubits"] == 10
    assert summary.result["hybrid"]["execution_device"] == "gpu"
    assert summary.config.hybrid_config.qaoa_depth == 1
    assert summary.config.hybrid_config.shots == 256
    assert summary.config.hybrid_config.optimizer_evaluations == 6
