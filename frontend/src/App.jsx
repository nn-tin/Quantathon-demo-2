...
function buildOptimizationRequest({ scenario }) {
  return {
    ...
    hybrid_config: {
      ...
      random_seed: stableSeedFromScenario(scenario),
      quantum_target:
        runtimeBackendConfig.quantumTarget ||
        import.meta.env?.VITE_QUANTUM_TARGET ||
        null,
      allow_numpy_fallback: true,
      rho_initial: 0.08,
      rho_growth: 1.8,
      rho_max: 2.0,
      residual_tolerance_mw: 0.25,
    },
    // The same runtime profiles are consumed by both Classical HiGHS and Hybrid QAOA.
...
function normalizeQuantathonRunResponse(payload, requestedSolver, scenario, elapsedSeconds) {
  ...
  const classicalMethod = runResult.classical || {};
  ...
  const operatingPlan = {
    ...
    summary: {
      ...
      comparison: runResult.comparison || {
    ...
    audit: {
      ...
      claim_boundary: "Classical HiGHS is the full-UC reference. Quantum optimizes only an 8–10-variable active block; every sampled schedule is reconstructed and classically validated.",
    },
  };
    ...
    <section className="resultsViewport pageEnter operatorResultsPage operatorDecisionBoard plainOperatorBoard">
      ...
      <section className={`operatorSection compactCommitmentSection operatorScheduleSection splitScheduleLayout ${scheduleView}`}>
        ...
        <OperatingEventsPanel
          ...
          footerActions={(
            <div className="actionLogFooterActions singleAction">
              <button type="button" className="secondaryCta" onClick={() => setIsComparisonOpen(true)}>
                Why this plan?
              </button>
            </div>
          )}
